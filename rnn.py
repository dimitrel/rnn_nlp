...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in reversed(range(len(x))):
			X = make_onehot(x[t],self.vocab_size)
			D = make_onehot(d[t],self.vocab_size)
			d_out=D-y[t]
			d_in =np.dot(self.W.T,d_out)*(s[t]*(1-s[t]))
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			self.deltaV += np.outer(d_in,X)
			self.deltaU += np.outer(d_in,s[t-1])
			for tr in range(1,steps+1):
				if(t-tr>=0):
					d_in = np.dot(self.U.T,d_in)*(s[t-tr]*(1-s[t-tr]))
					X = make_onehot(x[t-tr],self.vocab_size)
	...
	def compute_loss(self, x, d):
		...
		for t in range(len(y)):
			D = make_onehot(d[t],self.vocab_size)
			loss +=np.sum(-D*np.log(y[t]),axis=0)
	...
	def generate_sequence(self, start, end, maxLength,index=None):
		...
		for i in range(1,maxLength):
			...
			d=multinomial_sample(y[-1])
			if index!=None:
				while (d==index):
					d=multinomial_sample(y[-1])
			sequence.append(d)
			D = make_onehot(d,self.vocab_size)
...
if __name__ == "__main__":
	...
	if mode == "estimate":
		...
		X_dev_s = X_dev[:dev_size]
		D_dev_s = D_dev[:dev_size]
		...
		lr =[0.5,0.1,0.05]
		print("training...")
		for hidden_units in hdim:
			for backstep in lookback:
				for learning_rate in lr:
					model=RNN(vocab_size,hidden_units)
					loss=model.train(X, D, X_dev_s , D_dev_s, epochs=10, learning_rate=learning_rate, anneal=5, back_steps=backstep)
					print("\nEnd of training: Loss: {0}, Hidden units: {1}, Backsteps: {2}, Learning Rate: {3} \n".format(loss,hidden_units,backstep,learning_rate))
					if loss<best_loss:
						best_loss=loss
						best_hdim=hidden_units
						best_lr=learning_rate
						best_lookback =backstep
	...
	if mode == "train":
		...
		X_dev_s = X_dev[:dev_size]  # modification to run only a fraction of validation set in training
		D_dev_s = D_dev[:dev_size]
		...
		print("training...")
		model=RNN(vocab_size,hidden_units)
		model.train(X_train, D_train, X_dev_s, D_dev_s, epochs=10, learning_rate=0.5, anneal=5, back_steps=5)		
	 	run_loss = model.compute_mean_loss(X_dev,D_dev)
		np.save("rnn.U",model.U)
		np.save("rnn.V",model.V)
		np.save("rnn.W",model.W)
	...
	if mode == "generate":
		...
		unknown_index =	word_to_num["UUUNKKK"]

		sequence,loss=r.generate_sequence(start, end, maxLength,index=unknown_index)
		sentence=""
		for i in range(0,len(sequence)):
			sentence+=(num_to_word[sequence[i]])+" "
		print(sentence)
		print "Loss: %.03f" % loss
		print "Unadjusted: %.03f" % np.exp(loss)
